...
This program uses a GUI to allow a user to play a game of a match-three game called 'Taffy Tangle'. The user
can swap two adjacent pieces in hopes to match 3 or more taffies to earn points. If the user uses all their turns or
succeeds or fails to reach the target score the game ends. If no more valid moves are detected, the game will
auto end itself so the user is not left waiting for nothing.
...
def moves_left(board):
    """
    Analyzes the gae board array for any valid swap that could result in a match of at least three.
    :param board: The array representing the game board.
    :return: True, if any valid move is found throughout the board. False, if no moves can be found.
    """
    for r in range(len(board)):
        for c in range(1, len(board[r])):
            if r+1 in range(len(board)):
                if valid_swap(board, r, c, r+1, c):
                    return True, [r, c, r+1, c]
            if c+1 in range(len(board[0])):
                if valid_swap(board, r, c, r, c+1):
                    return True, [r, c, r, c+1]
            if r-1 in range(len(board)):
                if valid_swap(board, r, c, r-1, c):
                    return True, [r, c, r-1, c]
            if c-1 in range(len(board[0])):
...
def play_game(game_board, turns_left, target_score, max_piece, multiplier):
    score = 0
    while turns_left != 0 and moves_left(game_board)[0]:
        tTG.set_constants(turns_left, score, target_score)
        tTG.draw_board(game_board)
        hint = moves_left(game_board)[1]
        good_click = False
        while not good_click:
            mx, my = tTG.selection(hint)
            if mx == -1 and my == -1:
                break
            mx2, my2 = tTG.selection(hint)
            if are_adjacent(mx, my, mx2, my2):
                if valid_swap(game_board, mx, my, mx2, my2):
                    # Show a swap animation followed by physically changing the board array.
                    tTG.swap_animation(turns_left, score, target_score, game_board, mx, my, mx2, my2)
                    swap(game_board, mx, my, mx2, my2)
                    turns_left -= 1
                    score += clear_matches(game_board, multiplier)[0]
                    good_click = True
                else:
                    # The chosen pieces were adjacent but the move did not complete 3 in a row anywhere.
                    # Show a swap animation (from the tTG module twice).
                    for i in range(2):
                        tTG.swap_animation(turns_left, score, target_score, game_board, mx, my, mx2, my2)
                        swap(game_board, mx, my, mx2, my2)
                ...
                # Cancel the turn by ending the loop. No animations.
                good_click = True
        cleared = False
        time.sleep(0.1)
        while not cleared:
    ...
    tTG.play_again(turns_left, score, target_score, moves_left(game_board)[0])
